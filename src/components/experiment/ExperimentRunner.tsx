...
export function ExperimentRunner({ brands, keywords, models, onRunComplete }: ExperimentRunnerProps) {
  const [selectedVariants, setSelectedVariants] = useState<PromptVariant[]>(['minimal', 'frontloaded', 'stepwise']);
  const [runsPerCombination, setRunsPerCombination] = useState(3);
  ...
  const runExperiment = async () => {
    ...
    for (let i = 0; i <= 100; i += 5) {
      ...
      setProgress(i);
      setCurrentStep(`Processing ${Math.floor(i / 100 * totalCombinations)} / ${totalCombinations} combinations...`);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="glass-card p-6 animate-fade-in">
        <h3 className="font-semibold mb-4">Prompt Variants (CFF Conditions)</h3>
        <p className="text-sm text-muted-foreground mb-4">
          Based on Cognitive Forcing Functions research
        </p>
      ...
      <div className="glass-card p-6 animate-fade-in" style={{ animationDelay: '100ms' }}>
        <h3 className="font-semibold mb-4">Runs Per Combination</h3>
      ...
      <div className="glass-card p-6 animate-fade-in" style={{ animationDelay: '200ms' }}>
        <h3 className="font-semibold mb-4">Experiment Summary</h3>
